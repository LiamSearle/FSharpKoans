...
module ``10: Parametric polymorphism`` =
    ...
    *)
    ...
    type MyRecord <'x,'e> = {
        ...
        What : 'e // <-- should be generic, and a different type to Who
        Where : string
    }
    ...
    [<Test>]
    let ``05 Creating a generic discriminated union`` () =
        let a = Secnod (6.55, 7)
    ...
    [<Test>]
    let ``06 Creating a generic discriminate'FILL_ME_IN'd union`` () =
